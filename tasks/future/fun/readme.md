...
future::TryFuture<Response> Hedge(Request request) {
  ...
  return future::First(std::move(primary), std::move(backup));
}
``` 
...
// Producer
std::thread producer([p = std::move(p)] {
  std::move(p).Set(7);
});
...
Фьюча, представляющая первое значение / последнюю (комбинированную?) ошибку двух фьюч.
...
auto compute = future::Submit(pool, [] {
...
На вход комбинатору могут подаваться как fallible, так и infallible фьючи,
но с одним и тем же underlying value type `T`.

У `First` "плавающий" выходной тип:

- если на вход подается хотя бы один `TryFuture<T>`, то на выходе – `TryFuture<T>`,
- иначе – просто `Future<T>`.
...
```cpp
// Синхронно дожидаемся первого значения
auto r = (std::move(compute) or std::move(timeout)) | future::Get();
```

## Значения

Как вы можете видеть, пользовать работает с фьючами как с **иммутабельными значениями**, т.е. передает (точнее, перемещает)
