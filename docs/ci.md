...
Научимся отправлять решения задач на проверку.
...
Укажите в профиле свое полное имя.

#### Шаг 2

Зарегистрируйтесь в [Manytask](http://concurrency-course.cc/signup).
...
найти [здесь](https://docs.gitlab.com/ee/user/ssh.html).
...
Для этого на https://gitlab.com/ зайдите в `Preferences` своего профиля, затем в `Access Tokens` нажмите на `Add new token`, выберите произвольное имя для вашего токена, поставьте галку напротив `api`, установите expiration date и нажмите `Create personal access token`.
...
Здесь `{your-gitlab-token}` – токен, полученный вами на предыдущем шаге, без фигурных скобок.
