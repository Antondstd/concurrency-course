...
Развернем на локальном компьютере рабочее окружение для решения задач.
...
Теперь нужно установить 📎 [консольный клиент курса – Clippy](https://gitlab.com/Lipovsky/clippy).
...
Пролистайте документацию по командам `clippy`:
...
С помощью `clippy` сгенерируйте файлы сборки курса (курс – это проект `CMake`) и соберите общие библиотеки:
...
(К этому шагу можно вернуться позже)

Настройте IDE для комфортной работы с курсом: [Clion](/docs/ide/clion.md) (предпочтительно) или [VScode](/docs/ide/vscode.md).
...
Зарегистрируйтесь в Manytask, получите репозиторий для решений 
и научитесь посылать их на проверку: [CI](/docs/ci.md) 🤖